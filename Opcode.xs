...
/* Portablility is not just ppport.h */
#ifndef newSVpvn_flags
/* we only use SVs_TEMP */
# define newSVpvn_flags(name,length,flag) \
    sv_2mortal(newSVpvn(name,length))
#endif
#include "ppport.h"
#if PERL_VERSION < 8
char *
my_sv_2pv_flags(pTHX_ SV *sv, STRLEN *lp, I32 flags)
{
  STRLEN n_a = (STRLEN) flags;
  return sv_2pv(sv, lp ? lp : &n_a);
}
# undef sv_2pv_flags
# define sv_2pv_flags(sv,lp,flags) my_sv_2pv_flags(sv,lp,flags)
...
PPCODE:
	...
	for (i=0; i < MAXO; i++) {
            ...
            av_store(ref, 1, newSVpvn(PL_op_name[i],
                                      strlen(PL_op_name[i]) ));
