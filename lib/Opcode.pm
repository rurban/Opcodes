...
BEGIN {
	...
	opcodes opdesc opname ppaddr check opargs opclass opflags argnum
        opaliases opdump opname2code
	opmask define_optag opmask_add verify_opset
    );
}
...
# A handcoded list of ops without any SP handling (Note: stack_base is allowed),
...
sub opflags {
    ...
    $flags += 512 if $no_stack{opname($_[0])};
    return $flags;
}

# See F<opcode.pl> for $OASHIFT and $OCSHIFT. For flags n 512 we would have to change that.
sub opclass {
...
sub _init_optags {
	...
	my @ops   = map  { split ' ' } @lines; # get op words
        ...
        # filter out unsupported ops for this perl
        my @knownops;
	foreach(@ops) {
            push @knownops, ($_) if defined opname2code($_);
	}
	foreach(@knownops) {
	...
	define_optag($tag, opset(@knownops));
    }
    close(DATA);
...
Opcode - Opcode info and restricted sets for Safe
...
Functions for manipulating operator names tags and sets.
All are available for export by the package.
...
L<Safe> compartment (in I<addition> to any operators already permitted).

=item permit_only (OP, ...)

Permit I<only> the listed operators to be used when compiling code in
the L<Safe> compartment (I<no> other operators are permitted).

=item deny (OP, ...)

Deny the listed operators from being used when compiling code in the
L<Safe> compartment (other operators may still be permitted).

=item deny_only (OP, ...)

Deny I<only> the listed operators from being used when compiling code
in the L<Safe> compartment (I<all> other operators will be permitted).
...
Reini Urban added opcodes in list context and properties,
and dual-lifed it on CPAN, backported to older perls.
